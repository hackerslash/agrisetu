...
class _PaymentScreenState extends ConsumerState<PaymentScreen> {
  ...
  Future<void> _payNow() async {
    ...
    try {
      ...
      final allPaid = cluster.status == ClusterStatus.dispatched ||
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: clusterAsync.when(
        data: (cluster) => SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    ...
                    _PriceRow(
                      label: 'Your Share',
                      value:
                          '${cluster.members.isNotEmpty ? cluster.members.first.quantity.toStringAsFixed(0) : 0} ${cluster.unit}',
                    ),
                    const Divider(height: 24),
                    Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            '${cluster.members.where((m) => m.hasPaid).length} of ${cluster.membersCount} farmers paid · ${(cluster.members.where((m) => m.hasPaid).length / cluster.membersCount * 100).toStringAsFixed(0)}%',
                            style: AppTextStyles.label,
                          ),
                          Text(
